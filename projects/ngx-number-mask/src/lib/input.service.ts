...
export class InputService {
  ...
  constructor(
    ...
    private options: NgxNumberMaskConfig
  ) {
  ...
  addNumber(keyCode: number): void {
    const { decimal, decimalReplace, inputMode } = this.options;
    let keyChar = String.fromCharCode(keyCode);
    if (
      decimalReplace &&
      decimalReplace.length !== 0 &&
      keyChar === decimalReplace
    ) {
      keyChar = decimal;
    }
    ...
    if (!this.rawValue) {
      ...
      const precision = this.getPrecision(this.rawValue);
      let selectionStart: number = this.rawValue.length;
      if (inputMode === NumberMaskInputMode.NATURAL && precision > 0) {
      ...
      if (isDecimalOrThousands) {
        // If the cursor is just before the decimal or thousands separator and the user types the
        // decimal or thousands separator, move the cursor past it.
        this.rawValue = newValue;
        this.updateFieldValue(nextSelectionStart, true);
        return;
  ...
  applyMask(
    ...
    rawValue = isNumber
      ? new Number(rawValue).toString().replace('.', decimal)
      : rawValue.replace(decimalReplace, decimal);

    const precision = this.getPrecision(rawValue);
    ...
    if (inputMode === NumberMaskInputMode.NATURAL && !disablePadAndTrim) {
    ...
    if (precision > 0 && decimalPart.length > 0) {
    ...
    return prefix + operator + newRawValue + suffix;
  }

  private getPrecision(rawValue: string) {
    const { decimal, precision } = this.options;
    const decimalPart = rawValue
      .substring(
        rawValue.lastIndexOf(decimal) !== -1
          ? rawValue.lastIndexOf(decimal) + 1
          : rawValue.length
      )
      .toString();

    return Math.min(decimalPart.length, precision);
  }

  padOrTrimPrecision(rawValue: string): string {
    const { decimal } = this.options;
    const precision = this.getPrecision(rawValue);
    ...
    return (
      rawValue.substring(0, decimalIndex) +
      (decimalPortion.length ? decimal + decimalPortion : '')
    );
  }
  ...
  removeNumber(keyCode: number): void {
    ...
    let { selectionStart, selectionEnd } = this.inputSelection;
    ...
    const shiftSelection = 0;
    let insertChars = '';
    const isCursorInDecimals = decimalIndex < selectionEnd;
    const isCursorImmediatelyAfterDecimalPoint = decimalIndex === selectionEnd;
    if (selectionEnd === selectionStart) {
      if (keyCode === 8) {
        ...
        if (
          ...
          this.rawValue.substr(selectionStart, 1) !== decimal
        ) {
          selectionStart--;
        }

        // In natural mode, jump backwards when in decimal portion of number.
        if (inputMode === NumberMaskInputMode.NATURAL && isCursorInDecimals) {
          // shiftSelection = -1;
  ...
  set value(value: number | null) {
    if (typeof value === 'number') {
      this.rawValue = this.applyMask(true, value.toString());
    } else if (value) {
      const castedValue = value as string;
      this.rawValue = this.applyMask(
        false,
        castedValue.toString().replace('.', this.options.decimal)
      );
    } else {
      this.rawValue = null;
    }
